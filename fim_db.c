...
// #ifdef WIN32
    ...
    [FIMDB_STMT_REPLACE_REG_KEY] = "INSERT OR REPLACE INTO registry_key (path, perm, uid, gid, user_name, group_name, mtime, scanned, checksum, arch, rowid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
    [FIMDB_STMT_GET_REG_KEY] = "SELECT path, perm, uid, gid, user_name, group_name, mtime, scanned, checksum, arch FROM registry_key WHERE path = ?;",
    ...
    [FIMDB_STMT_UPDATE_REG_KEY] = "UPDATE registry_key SET perm = ?, uid = ?, gid = ?, user_name = ?, group_name = ?, mtime = ?, scanned = ?, checksum = ?, arch = ? WHERE path = ?;",
    [FIMDB_STMT_GET_ALL_REG_ENTRIES] = "SELECT path, perm, uid, gid, user_name, group_name, mtime, registry_key.scanned, registry_key.checksum, arch, key_id, name, type, size, hash_md5, hash_sha1, hash_sha256, registry_data.scanned, last_event, registry_data.checksum FROM registry_data INNER JOIN registry_key ON registry_key.rowid = registry_data.key_id ORDER BY PATH ASC;",
    [FIMDB_STMT_GET_REG_KEY_NOT_SCANNED] = "SELECT path, perm, uid, gid, user_name, group_name, mtime, scanned, checksum, arch FROM registry_key WHERE scanned = 0;",
...
static void fim_db_bind_insert_registry_key(fdb_t *fim_sql, fim_registry_key *registry_key, const unsigned int rowid) {
    ...
    if (rowid == 0) {
        sqlite3_bind_null(fim_sql->stmt[FIMDB_STMT_REPLACE_REG_KEY], 11);
    } else {
...
fim_registry_value_data *fim_db_decode_registry_data_row(sqlite3_stmt *stmt) {
    ...
    strncpy(value->hash_sha1, (char *)sqlite3_column_text(stmt, 5), sizeof(os_sha1) - 1);
    strncpy(value->hash_sha256, (char *)sqlite3_column_text(stmt, 6), sizeof(os_sha256) - 1);
    value->scanned = (unsigned int)sqlite3_column_int(stmt, 7);
    value->last_event = (unsigned int)sqlite3_column_int(stmt, 8);
    strncpy(value->checksum, (char *)sqlite3_column_text(stmt, 9), sizeof(os_sha1) - 1);
...
fim_registry_value_data *fim_db_get_registry_data(fdb_t *fim_sql, const unsigned int key_id, const char *name) {
    ...
    if (sqlite3_step(fim_sql->stmt[FIMDB_STMT_GET_REG_DATA]) == SQLITE_ROW) {
        value = fim_db_decode_registry_data_row(fim_sql->stmt[FIMDB_STMT_GET_REG_DATA]);
    }
...
fim_registry_key *fim_db_get_registry_key(fdb_t *fim_sql, const char *path) {
    ...
    if (sqlite3_step(fim_sql->stmt[FIMDB_STMT_GET_REG_KEY]) == SQLITE_ROW) {
        reg_key = fim_db_decode_registry_key_row(fim_sql->stmt[FIMDB_STMT_GET_REG_KEY]);
    }
...
int fim_db_insert_registry(fdb_t *fim_sql, fim_entry *new) {
    int res_data = 0;
    int res_key = 0;
    //if (fim_db_get_registry_key_rowid(fim_sql, new->registry_entry.key->path, &row_id) == FIMDB_OK) {
